...
bool compareByCity(const Addresses* addr1, const Addresses* addr2)
{
    return addr1->getCity() < addr2->getCity();
}
...
int main()
{
    ...
    for (int i = 0; i < countAddr - 1; ++i)
    {
        for (int j = 0; j < countAddr - i - 1; ++j)
        {
            if (!compareByCity(addresses[j], addresses[j + 1]))
            {
